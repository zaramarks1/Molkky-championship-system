...
public interface MatchRepository extends JpaRepository<Match, String>, JpaSpecificationExecutor<Match> {
    ...
    @Query(value="SELECT DISTINCT(m) FROM Match m JOIN m.round.phase.tournament t WHERE t=:tournament AND m.staff=:staff")
    List<Match> findMatchAttributedToStaff(@Param("tournament")Tournament tournament,@Param("staff") User staff);

    @Query(value="SELECT DISTINCT(m) FROM Match m JOIN m.round.phase.tournament t WHERE t=:tournament AND m.finished=:finished AND m.staff=:staff")
