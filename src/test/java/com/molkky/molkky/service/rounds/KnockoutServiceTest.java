...
@SpringBootTest(classes = MolkkyApplication.class)
 class KnockoutServiceTest {
    ...
    void testInsertTournamentWithKnockoutRoundWith2matches() {
        ...
        Assertions.assertEquals(4, tournament.getPhases().get(0).getRounds().get(0).getMatches().size(),
                " there should be 4 matches in the first round");


        Assertions.assertEquals(1, tournament.getTeams().get(5).getMatchs().size(), " There should be 2 matches for team 6 ");
        Assertions.assertEquals(1, tournament.getTeams().get(7).getMatchs().size(), " There should be 2 matches for team 8 ");
        Assertions.assertEquals(1, tournament.getTeams().get(6).getMatchs().size(), " There should be 2 matches for team 7 ");
        ...
        for(Map.Entry<Round, List<Match>> entry : results.entrySet()){
            ...
            Assertions.assertEquals(4, entry.getValue().size(), " The  should be 4 matches");
